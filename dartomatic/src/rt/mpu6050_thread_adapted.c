...
/*
 * IMPORTANT :
 *  - arg doit être un MpuAdapter*
 *  - le thread notifie un impact via mpu_adapter_notify_impact()
 */
void* mpu_thread(void* arg)
{
    MpuAdapter* adapter = (MpuAdapter*)arg;
    ...
    if (log) fprintf(log, "t_ms,ax,ay,az,norm,dnorm,energy,impact\n");
    ...
    while (1) {
        ...
        (void)ahp; /* ahp pas utilisé dans ta version actuelle, mais on le garde */
            ...
            (t - last_impact) > LOCKOUT_MS)
        {
            ...
            /* ✅ Nouvelle sortie : notification via pipe */
            if (adapter) {
                mpu_adapter_notify_impact(adapter);
            }
        }
